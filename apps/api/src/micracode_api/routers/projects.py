"""Project CRUD + file/prompt hydration endpoints.

No auth: the API binds to 127.0.0.1 and treats the local user as trusted.
Every ``{id}`` path param is validated by ``SlugPath`` to match the slug
regex, and the storage layer path-checks every file operation.
"""
...
from ..storage import SLUG_RE, SNAPSHOT_ID_RE, iter_ignored_top_level
...
def _reject_sidecar_path(rel: str) -> None:
    if rel == ".micracode" or rel.startswith(".micracode/"):
        raise HTTPException(
            status_code=400,
            detail="cannot write under .micracode",
        )
...
async def download_project_zip(
    ...
    """Return the project's source tree packaged as a zip archive.

    Excludes the same top-level entries as ``read_tree`` (``.micracode``,
    ``node_modules``, build output, etc.) so the download mirrors what the
    user sees in the editor. Files live under a ``{slug}/`` prefix so the
    archive unzips into a single directory.
    """
...
async def pop_last_assistant_prompt(
    ...
    """Strip the last assistant row from ``prompts.jsonl``.

    Used by the retry flow so a re-issued codegen call replaces the bad
    reply rather than piling up duplicates. Returns ``{popped: true}`` if
    a row was removed, ``{popped: false}`` if the history was already
    empty or ended on a non-assistant row.
    """
